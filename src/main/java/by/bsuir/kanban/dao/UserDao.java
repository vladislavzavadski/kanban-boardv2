...
public interface UserDao extends JpaRepository<User, String>{
    ...
    @Query("select (count (us) > 0) from User us join us.userProjects proj where us.username = :username " +
            "and proj.project.id = :id")
    ...
    @Query("select (count (us) > 0) from User us join us.userProjects up join up.project pr where pr.id = " +
            "(select proj.id from Project proj join proj.tasks tasks where tasks.id = :taskId) and us.username = :username")
    boolean isAssignedOnProjectByTaskId(@Param("username") String username, @Param("taskId") int taskId);

    @Query("select (count (us) > 0) from User us join us.userProjects up join up.project pr where pr.id = " +
            "(select pr.id from Project pr join pr.userProjects up join up.permissions perm where perm.id = :permissionId) and us.username = :username")
    ...
    @Query("select (count (us) > 0) from User us join us.userProjects up join up.project pr where pr.id = " +
            "(select proj.id from Project proj join proj.statuses stats where stats.id = :statusId) and us.username = :username")
    boolean isProjectOwnerByStatusId(@Param("username") String username, @Param("statusId") int statusId);
    ...
    boolean isProjectOwnerByTaskId(@Param("username") String username, @Param("taskId") int taskId);

    @Query("select (count (us) > 0) from User us where us.email = :email")
